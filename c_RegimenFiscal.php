<!-- BEGIN BASE-->
		...
		$ide=$_GET['id'];
		$query="SELECT*FROM c_RegimenFiscal where id='$ide'";
		$res=mysqli_query($conn,$query)or die(mysqli_error($conn));
		$r=mysqli_fetch_array($res);
		$nome=$r['Descripcion'];
		$regimen=$r['c_RegimenFiscal'];
		$fisica=$r['fisica'];
		$motal=$r['Motal'];
		$fechai=$r['fecha_InVigencia'];
		$fechaf=$r['fecha_FinVigencia'];
		}
		?>
		<div id="base">
			...
			<div id="content">
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							<li><a href="?modulo=c_RegimenFiscal"><h4>Recursos humanos / Datos maestros / Regimen fiscal</h4></a></li>
							<?php 
						if($accion=='agregar'){
							echo"<li class='active'>Añadir Tipo regimen fiscal</li>";
						}
						if($accion=='editar'){
							echo"<li class='active'>Editar Tipo regimen fiscal</li>";
						}
						if($accion=='ver'){
							echo"<li class='active'>Ver Tipo regimen fiscal</li>";
						}
						if($accion=='borrar'){
							echo"<li class='active'>Borrar Tipo regimen fiscal</li>";
						}
						?>
						</ol>
					</div>
							...
							<div class="card-head style-primary">
								...
								<div class="card-body">
										<div class="form-group floating-label">
											...
											<label for="regular4">Regimen fiscal</label>
										</div>
										...
										<div class="form-group floating-label">
												<input type="text" class="form-control" name="fisica" id="fisica" value="<?php echo $fisica; ?>">
											<label for="regular4">Persona fisica</label>
										</div>
										<div class="form-group floating-label">
												<input type="text" class="form-control" name="motal" id="motal" value="<?php echo $motal; ?>">
											<label for="regular4">Persona moral</label>
										</div>
...
<script src="styles/js/c_RegimenFiscal/c_RegimenFiscal_Agregar.js"></script>
<!-- Clases -->
<script>
	   $(function () {
